...
class SQLMap(object):
    ...
    def begin(self, cursor_factory=None, isolation_level=None):
        conn = self.conn
        if conn is None:
        ...
        self.curs = conn.cursor(cursor_factory=cursor_factory)  # allows only string keys like a real dict
    ...
    def get_colnames(self):
        curs = self.curs
        if curs is not None and len(curs.description)>0:
            return [col[0] for col in curs.description]
        else:
            return None